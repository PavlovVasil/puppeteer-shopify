...
const saveUrlsToFile = async (urls = [], fName = "default.json") => {
    await fs.writeFile(fName, JSON.stringify({urls: urls}));
}
...
const getExpertUrlsForSection = async (baseUrl, sectionUrl, browser) => {
    ...
    expertUrls = expertUrls.concat(Array.from($('.kVWrj a')).map(a => `${baseUrl}${$(a).prop('href')}`));
    
    //if the nextButton is disabled, we are on the last page of this section
    let nextButton = !$('.Polaris-Pagination__Button.Polaris-Pagination__NextButton').disabled
    ...
    while(nextButton){
        ...
        const $ = cheerio.load(content);
        expertUrls = expertUrls.concat(Array.from($('.kVWrj a')).map(a => `${baseUrl}${$(a).prop('href')}`));
        nextButton = !$('.Polaris-Pagination__Button.Polaris-Pagination__NextButton').disabled
        console.log(expertUrls)
        i++
    }
    return expertUrls
}
...
(async () => {
    try {
        ...
        //this saves the section urls to a json file
        ...
        await getExpertUrlsForSection(baseUrl, baseSectionUrl, browser);
        await browser.close()
    } catch (e) {
